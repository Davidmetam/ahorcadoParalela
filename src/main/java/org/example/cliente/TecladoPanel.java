...
public class TecladoPanel extends JPanel {
    ...
    public void desactivarLetra(char letra) {
        ...
        if (letra >= 'A' && letra <= 'Z') {
            ...
            if (indice >= 0 && indice < botones.length) {
                JButton button = botones[indice];
                button.setEnabled(false);
                button.setForeground(new Color(80, 80, 80));
                button.repaint();
            }
        } else {
            System.out.println("Carácter no válido recibido: " + letra);
        }
    }

    // Método para habilitar todas las teclas (reinicio del juego)
    public void resetearTeclado() {
        ...
        System.out.println("Teclado reiniciado");
    }
}